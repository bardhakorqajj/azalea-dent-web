...
export function Intro({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    <section id="about" className="bg-bone-100 dark:bg-ink-900">
      <div className="grid lg:grid-cols-2">
        ...
        <div className="flex items-center px-6 py-16 sm:px-10 lg:px-16 lg:py-24 xl:px-24">
          <div className="w-full">
            ...
            <Reveal delay={80}>
              <div className="mt-7 space-y-5 text-[1.0625rem] leading-relaxed text-ink-600 dark:text-bone-300">
            ...
            <Reveal delay={140}>
              <dl className="mt-10 flex flex-wrap gap-x-10 gap-y-6 border-t border-ink-900/10 pt-8 dark:border-bone-100/12">
                {dict.intro.stats.map((stat) => (
                  <div key={stat.label}>
                    <dt className="sr-only">{stat.label}</dt>
                    <dd>
                      <span className="block font-display text-[1.9rem] leading-none whitespace-nowrap text-ink-900 sm:text-[2.4rem] dark:text-bone-50">
                        {stat.value}
                      </span>
                      <span className="eyebrow mt-3 block text-ink-500 dark:text-bone-300">
                        {stat.label}
                      </span>
                    </dd>
                  </div>
