...
export function MobileActionBar({
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-ink-900/10 bg-bone-50/97 backdrop-blur-md lg:hidden">
      <div className="flex items-center gap-2 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <Link
          href={path(locale, "/appointment")}
          className="flex min-h-12 flex-1 items-center justify-center rounded-sm bg-ink-900 px-4 text-[0.72rem] font-medium tracking-[0.14em] text-bone-50 uppercase"
        >
