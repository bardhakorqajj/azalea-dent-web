...
/** The clinic mark as a data URI — Satori renders SVG reliably through <img>. */
function markDataUri(): string {
  ...
  const rays = [36, 108, 180, 252, 324]
    .map(
      (angle) =>
        `<g transform="rotate(${angle} 50 50)" stroke-width="2.2">` +
        `<path d="M50 26.3 V17.1"/><path d="M50 13 V10.4"/></g>`,
    )
    .join("");

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="200" height="200"><g fill="none" stroke="#e3b657" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round">${teeth}${rays}</g></svg>`;
